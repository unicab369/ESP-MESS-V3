...
esp_err_t mod_nvs_erase(const char *key) {
	...
	if (ret != ESP_OK) {
		ESP_LOGE(TAG_NVS, "Err mod_nvs_erase: %s", esp_err_to_name(ret));
		return ret;
	}

	ret = nvs_commit(NVS_HANDLER);
	if (ret == ESP_OK) {
		ESP_LOGW(TAG_NVS, "Value with key %s erased", key);
	}
...
esp_err_t mod_nvs_erase_all(void) {
	esp_err_t ret = nvs_erase_all(NVS_HANDLER);
	if (ret != ESP_OK) {
		ESP_LOGE(TAG_NVS, "Err mod_nvs_erase_all: %s", esp_err_to_name(ret));
		return ret;
	}

	ret = nvs_commit(NVS_HANDLER);
	if (ret == ESP_OK) {
		ESP_LOGW(TAG_NVS, "Done mod_nvs_erase_all");
	}
...
esp_err_t mod_nvs_set_blob(const char *key, void* value, size_t len) {
	esp_err_t err = nvs_set_blob(NVS_HANDLER, key, value, len);
