...
function initCharts() {
	...
	const historyOpts = {
		...
		width: document.getElementById('historyChart').offsetWidth,
		height: 280,
		scales: {
			x: { time: true },
			y: { 
	...
	historyChart = new uPlot(historyOpts, historyData.data, document.getElementById('historyChart'));
}
...
function updateCharts() {
	...
	// Calculate statistics for history chart
	const windowSize = 10;
	...
	for (let i = 0; i < dataPoints.length; i += windowSize) {
		const chunk = dataPoints.slice(i, i + windowSize);
		if (chunk.length === 0) continue;
		
		historyTimestamps.push(chunk[chunk.length - 1].timestamp);
		minValues.push(Math.min(...chunk.map(p => p.value)));
		maxValues.push(Math.max(...chunk.map(p => p.value)));
		avgValues.push(chunk.reduce((sum, p) => sum + p.value, 0) / chunk.length);
	}
	
	// Update history chart
	historyChart.setData([historyTimestamps, minValues, maxValues, avgValues]);
}
