...
function updateDeviceList(device_caches, device_configs) {
	...
	if (devices.length === 0) {
		...
		devices.forEach(device => {
			...
			const logged_objs = {
				color: device.config > 0 ? '#4CAF50' : '#f44336',
				text: device.config > 0 ? 'Logging' : 'Not Log'
			}
			
			html += /*html*/
				`<div style="display: flex; align-items: center; padding: 8px; border-bottom: 1px solid #eee; gap: 10px;">
					<div style="flex: 1;">
						<div style="font-weight: bold;">${device.uuid}</div>
						<div style="font-size: 0.8rem; color: #666;">
							${device.name} • ${timeAgo}s ago
						</div>
					</div>
